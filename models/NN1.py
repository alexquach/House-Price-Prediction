"""
1-Hidden Layer Neural Network (Tensorflow)
...
#Weights and global_variables_initializer
...
#msle_loss with L2 regularization
msle_loss = tf.reduce_mean(tf.square( tf.log(y) - tf.log(y_) ))
msle_loss = msle_loss + regularizer * beta
...
for _ in range(100000):
    ...
    a = sess.run(msle_loss, feed_dict={x: validate, y_: validate_result, keep_prob: 0.5})
    ...
    if( _ % 1000 == 0):
        print("Epoch" + str(_) )
        ww = sess.run(w, feed_dict={x: train_sub, y_: train_sub_result, keep_prob: 0.5})
        ww1 = sess.run(w1, feed_dict={x: train_sub, y_: train_sub_result, keep_prob: 0.5})
...
pd.DataFrame({'Id': nparange, 'SalePrice': tfresult}).to_csv('NN1.csv', index =False)
